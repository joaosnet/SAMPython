...
def testbench():
    ...
    sd_inst = sd(Reset, ck, entA, entB, mult, pronto)
    ...
    def stimulus():
        ...
        entA.next = 4
        entB.next = 3
        yield delay(135)  # noqa: F405
        raise StopSimulation  # noqa: F405

    @instance  # noqa: F405
    def monitor():
        print("ck entA entB mult pronto")
        print("-------------------------")
        while True:
            yield ck.posedge
            print(f"{int(ck)}  {int(entA)}   {int(entB)}   {int(mult)}   {int(pronto)}")
...
def convert():
    ...
    mult = Signal(intbv(0)[4:])  # noqa: F405
    pronto = Signal(bool(0))  # noqa: F405
    convInst = sd(Reset, ck, entA, entB, mult, pronto)
